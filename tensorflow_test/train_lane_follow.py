...
xs = tf.placeholder(tf.float32, shape=(None,101,101,3), name="x") # 28x28
ys = tf.placeholder(tf.float32, shape=(None,15), name="y")
keep_prob = tf.Variable(0.8,name="keep_prob")
...
train_step = tf.train.AdamOptimizer(5*(1e-5),name='opt').minimize(cross_entropy)
...
el = iter.get_next()
#constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ['fc2'])
for i in range(5000):
