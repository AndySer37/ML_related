...
# 定義placeholder
xs = tf.placeholder(tf.float32, shape=(480,640,3)) # 28x28
ys = tf.placeholder(tf.float32, shape=(10))
keep_prob = tf.placeholder(tf.float32)
 
#將圖片reshape， -1表示會自動算幾組
#28,28,1 分別代表寬 高 channel數(像RGB的話這裡就要改3)
x_image = tf.reshape(xs, [-1, 480, 640, 3])
 
 
#開始組裝神經網路
## conv1 layer ##
#1:表示 input_size  32:表示output_size 所以這裡表示一張圖總共訓練出32個filter
W_conv1 = weight_variable([4,4, 3,32]) # patch 5x5, in size 1, out size 32
