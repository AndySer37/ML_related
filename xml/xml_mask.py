...
"""
brand_list = ['background','3m', 'andes', 'cocacola', 'crayola', 'folgers','heineken','hunts','kellogg','kleenex',\
               'kotex','libava','macadamia','milo','mm','pocky','raisins','stax','swissmiss','vanish','viva','barcode']
    ...
    if 'object' in label_dict['annotation']:
        if type(label_dict['annotation']['object']).__name__ != "list":
            try:
                ...
                if  object_['name'][4:] in brand_list and object_['deleted'] == '0':
                    poly_vertice = []
                    ...
                    for i in range(len(brand_list)):
                        if brand_list[i] == object_['name'][4:]:
                            cv2.fillConvexPoly(mask, poly_vertice, i*12)
